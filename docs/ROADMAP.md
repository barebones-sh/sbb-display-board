...
- Chromium in kiosk mode as the display surface. This is already a
  regular web app, so no rewrite is needed, just a launcher/provisioning
...
**Per-row rerouting text stays mocked**, an independent gap, still open. A
...
- **RE badges always show a route number** (e.g. "RE80") even though the
  FR/DE reference images show a bare "RE" for some services. This was a
  deliberate call made during bootstrap to keep badge rendering fully
  data-driven rather than special-casing one category; see the "RE badge
  format" decision in the bootstrap plan for the reasoning.
